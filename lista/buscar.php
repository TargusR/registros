...
<html>
	<head>
		...
		<script type='text/javascript'>
			...
			function eliminar(id){
				url = parent.document.getElementById('Sender').src;
				url = url.split('?');
				parent.document.getElementById('Sender').src = url[0] + "?del=" + id + "&ident=0";
	...
	<body>
		<?php
		if($columna) {
			if($busqueda == "") $sql="SELECT * FROM articulos ORDER BY articulos.$columna";
			else $sql="SELECT * FROM articulos WHERE articulos.$columna LIKE '%$busqueda%' ORDER BY articulos.$columna";
			$_SESSION['busqueda'] = $sql;
			...
			if(mysql_num_rows($consult)!=0){
				...
				if ($acceso=='admin' || $acceso=='ventas'|| $acceso == 'agendados' || $acceso == 'guias') echo "<table id='Tabla-Resultado'><tr><th>#ID</th><th>Imagen</th><th>Nombre</th><th>Precio 1</th><th class='extra'>Precio 2</th><th class='extra'>Precio 3</th><th class='extra'>Precio 4</th><th class='extra'>Precio 5</th><th class='extra'>C. Magno Inv.</th><th class='extra'>C. Magno Ped.</th><th class='extra'>Plaza T. 05 Inv</th><th class='extra'>Plaza T. 05 Ped.</th><th class='extra'>Plaza T. 177 Inv.</th><th class='extra'>Plaza T. 177 Ped.</th><th class='extra'>Macro Tienda Inv.</th><th class='extra'>Macro Tienda Ped.</th><th class='extra'>Proveedores</th><th>Categoria</th><th>Subcategoria</th><th>Actualizado</th><th></th><th></th></tr>";
				...
				while($row = mysql_fetch_assoc($consult)){
					...
					if ($acceso == 'ventas' || $acceso == 'agendados' || $acceso == 'guias' ){
						echo "<tr><td>".$row['id']."</td><td><div class='imageContainer'><img id='img".$row['id']."' src='".$row['imagen']."' onload='resize(&#34;img".$row['id']."&#34;);'></div></td><td>".$row['nombre']."</td><td>$".$row['precio1']."</td><td class='extra'>$".$row['precio2']."</td><td class='extra'>$".$row['precio3']."</td><td class='extra'>$".$row['precio4']."</td><td class='extra'>$".$row['precio5']."</td><td class='extra'>".$row['inv_CM']."</td><td class='extra'>".$row['ped_CM']."</td><td class='extra'>".$row['inv_PT05']."</td><td class='extra'>".$row['ped_PT05']."</td><td class='extra'>".$row['inv_PT177']."</td><td class='extra'>".$row['ped_PT177']."</td><td class='extra'>".$row['inv_MT']."</td><td class='extra'>".$row['ped_MT']."</td><td class='extra'>".$row['proveedores']."</td><td>".$row['categoria']."</td><td>".$row['subcategoria']."</td><td>".$row['actualizado']."</td>";	
					}
					if($acceso == 'almacen'){
						echo "<tr><td>".$row['id']."</td><td><div class='imageContainer'><img id='img".$row['id']."' src='".$row['imagen']."' onload='resize(&#34;img".$row['id']."&#34;);'></div></td><td>".$row['nombre']."</td><td>$".$row['precio1']."</td><td class='extra'>$".$row['precio2']."</td><td class='extra'>$".$row['precio3']."</td><td class='extra'>$".$row['precio4']."</td><td class='extra'>$".$row['precio5']."</td><td class='extra'>".$row['inv_CM']."</td><td class='extra'>".$row['ped_CM']."</td><td class='extra'>".$row['inv_PT05']."</td><td class='extra'>".$row['ped_PT05']."</td><td class='extra'>".$row['inv_PT177']."</td><td class='extra'>".$row['ped_PT177']."</td><td class='extra'>".$row['inv_MT']."</td><td class='extra'>".$row['ped_MT']."</td><td class='extra'>".$row['proveedores']."</td><td>".$row['categoria']."</td><td>".$row['subcategoria']."</td><td>".$row['actualizado']."</td>";	
		...
		} else echo "<div class='alert'><p>Ingrese palabras clave para iniciar la búsqueda</p></div>";
		?>
	</body>
</html>
