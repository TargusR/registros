	...
	<?php
		...
		switch($tabla) {
			...
			case 'PENDIENTES': // Tabla Recientes
				...
				// Ensamblar Filtro
				if($filtro)
					$filtro = " where a.pedido like '%". $filtro ."%' || a.encargado like '%". $filtro ."%' || a.comentario like '%". $filtro ."%' || c.correo like '%". $filtro ."%' || c.vendedor like '%". $filtro ."%'";
				
				//obtener el total de registros
				$sql = "SELECT count(*) as total FROM ". $table_name ." AS a JOIN ( SELECT max(ID) AS entrada, max(estado) AS terminado FROM ". $table_name ." GROUP BY pedido) as b on a.id = b.entrada && b.terminado = 0 LEFT JOIN (SELECT pedido as pedido_p, correo, vendedor FROM paquetes) AS c on a.pedido = c.pedido_p". $filtro .";";
				...
				$sql = "SELECT * FROM ". $table_name ." AS a JOIN ( SELECT max(ID) AS entrada, max(estado) AS terminado FROM ". $table_name ." GROUP BY pedido) as b on a.id = b.entrada && b.terminado = 0 LEFT JOIN (SELECT pedido as pedido_p, correo, vendedor FROM paquetes) AS c on a.pedido = c.pedido_p". $filtro ." ORDER BY ". $orderBy ." ". $orderBy_order ." LIMIT ". $consultas * $indice .", ". $consultas .";";
				$consult = mysql_query($sql, $conex) or die(mysql_error());
				...
				if(mysql_num_rows($consult)!=0){
					...
					echo "<table id='Tabla-Resultado'><tr>";
						...
						if ($orderBy == "vendedor") echo "<th class='ordered , ". $orderBy_order ."'>";
						else echo "<th>";
						echo "<a href='?indice=". $indice ."&orderBy=vendedor'>Vendedor</a></th>";
						
					echo "</tr>";
					
					while($row = mysql_fetch_assoc($consult)){
						echo "<tr><td>".$row['fecha']."</td><td>".$row['pedido']."</td><td> ".$row['correo']." </td><td>".$row['encargado']."</td><td>".$row['asunto']."</td><td>".$row['comentario']."</td><td> ".$row['vendedor']." </td></tr>";
			...
			case 'PERSONALES': // Tabla Recientes
				...
				$sql = "SELECT count(*) as total FROM ". $table_name ." AS a JOIN ( SELECT max(ID) AS entrada, max(estado) AS terminado FROM ". $table_name ." GROUP BY pedido) as b on a.id = b.entrada && b.terminado = 0 JOIN (SELECT pedido as pedido_p, correo, vendedor FROM paquetes WHERE vendedor = '". $usuario ."') AS c on a.pedido = c.pedido_p". $filtro .";";
				$consult = mysql_query($sql, $conex) or die(mysql_error());
				$consult = mysql_fetch_assoc($consult);
				$total = $consult['total'];
				unset($consult);
