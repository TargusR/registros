...
-----------------------*/
	...
	$parent_table_name = 'cerrado';
	$valid_states = array(
		"Paquete Recogido"
		); // Estados definidos como aprovados para las publicaciones
	$estado_label_next = "Envío en Curso";
	$estado_label_act = "Esperando Recolección";
...
<html>
		...
		<script type='text/javascript'>
			...
			function addForm_registrar(fila,num_pedido){
				...
				asunto.name = 'asunto';
					...
					option.value = 'Paquete Recogido';
					option.appendChild(document.createTextNode('Paquete Recogido'));
					asunto.appendChild(option);
					
					option = document.createElement('option');
					option.value = 'No coincide Guía';
					option.appendChild(document.createTextNode('No coincide Guía'));
					asunto.appendChild(option);
	...
	<body class='dobleCetrado' 
		<?php
		if(!$alert)
			echo "onLoad='setTimeout(limpiar(parent.document.getElementById(&quot;recoleccion-form&quot;)),5);'";
		?>
	>
	...
	<?php
		...
		if($tabla == 'HEREDADOS') echo "<span>Nuevos Cerrados</span>";
		else echo "<a href='?tabla=HEREDADOS'>Nuevos Cerrados</a>";
		if($tabla == 'RECIENTES') echo "<span>Bitacora Recolección</span>";
		else echo "<a href='?tabla=RECIENTES'>Bitacora Recolección</a>";
			...
			case 'HEREDADOS': // Tabla Recientes
				...
				if(mysql_num_rows($consult)!=0){
					...
					while($row = mysql_fetch_assoc($consult)){
						echo "<tr><td>".$row['fecha']."</td><td>".$row['pedido']."</td><td>".$row['correo']."</td><td>".$row['guia']."</td><td>".$row['encargado']."</td><td>".$row['asunto']."</td><td>".$row['comentario']."</td>";
						...
						echo "<td><a href='#' onClick='addForm_registrar(this.parentNode.parentNode,\"". $row['pedido'] ."\"); return false;'>Marcar Recogido</a></td>";
