...
-----------------------*/
	...
	$table_name = 'impresos';
	$parent_table_name = 'cotejados';
	$valid_states = array(
		"Guía Asignada (DHL)",
		"Guía Asignada (ESTAFETA)",
		"Cambio de Guía (DHL)",
		"Cambio de Guía (ESTAFETA)"
		); // Estados definidos como aprovados para las publicaciones
	$estado_label_next = "Pedido en Almacen";
	$estado_label_act = "Pago revisado";
	...
	Switch($ident){
		...
		case 'REGISTER': //operación de inserción
			...
			if(mysql_num_rows($paquete)==0) {
				...
				// comprobar si hay indicador de segunda guía
				$pedido = explode('_', $pedido);
				...
				$paquete_root = "SELECT * from paquetes WHERE paquetes.pedido = '". $pedido[0] ."';";
				$paquete_root = mysql_query($paquete_root, $conex) or die(mysql_error());
				if(mysql_num_rows($paquete_root)==0) {
					// Alertar que el pedido no existe, impedir registros fantasma
					$msg = "<p>Error: El pedido <b>". $pedido[0] ."</b> no tiene un registro raiz y no se puede extender</p>";
					$alert = true;
					break;
				}
				
				
				// registrar paquete, heredar información
				$paquete_root = mysql_fetch_assoc($paquete_root);
				
				$pedido = implode('_', $pedido);
				$correo = $paquete_root["correo"];
				$finicio = $paquete_root["finicio"];
				$vendedor = $paquete_root["vendedor"];
				
				$sql[] = "INSERT INTO paquetes (pedido, correo, finicio, fultima, estado, vendedor) VALUES('$pedido', '$correo', '$finicio', '$timestamp', '$estado_label', '$vendedor');";
			...
			if($guia) // Añadir Guia al paquete
				$sql[] = "UPDATE paquetes SET guia = '$guia' WHERE pedido = '$pedido';";
...
<html>
	<head>
		...
		<script type='text/javascript'>
			...
			function addForm_registrar(fila,num_pedido,guia_num){
				...
				asunto.name = 'asunto';
					...
					option.value = 'Guía Asignada (DHL)';
					option.appendChild(document.createTextNode('Guía Asignada (DHL)'));
					asunto.appendChild(option);
					
					option = document.createElement('option');
					option.value = 'Guía Asignada (ESTAFETA)';
					option.appendChild(document.createTextNode('Guía Asignada (ESTAFETA)'));
					asunto.appendChild(option);
					
					option = document.createElement('option');
					option.value = 'Guía Pendiente';
					option.appendChild(document.createTextNode('Guía Pendiente'));
					asunto.appendChild(option);
					
					option = document.createElement('option');
					option.value = 'Cambio de Guía (DHL)';
					option.appendChild(document.createTextNode('Cambio de Guía (DHL)'));
					asunto.appendChild(option);
					
					option = document.createElement('option');
					option.value = 'Cambio de Guía (ESTAFETA)';
					option.appendChild(document.createTextNode('Cambio de Guía (ESTAFETA)'));
					asunto.appendChild(option);
				...
				label = document.createElement('span');
				label.appendChild(document.createTextNode('Guia'));
				celdaForm.appendChild(label);
				filaForm.appendChild(celdaForm);
				
				celdaForm = document.createElement('td');
				
				guia = document.createElement('input');
				guia.type = 'TEXT';
				guia.name = 'guia';
				if(guia_num != 0)
					guia.value = guia_num;
				celdaForm.appendChild(guia);
				filaForm.appendChild(celdaForm);
	...
	<body class='dobleCetrado' 
		<?php
		if(!$alert)
			echo "onLoad='setTimeout(limpiar(parent.document.getElementById(&quot;impresos-form&quot;)),5);'";
		?>
	>
	...
	<?php
		...
		if($tabla == 'HEREDADOS') echo "<span>Cotizaciones Nuevas</span>";
		else echo "<a href='?tabla=HEREDADOS'>Cotizaciones Nuevas</a>";
		if($tabla == 'RECIENTES') echo "<span>Bitacora Impresos</span>";
		else echo "<a href='?tabla=RECIENTES'>Bitacora Impresos</a>";
		...
		switch($tabla) {
			...
			case 'HEREDADOS': // Tabla Recientes
				...
				$sql="SELECT count(*) as total FROM ". $parent_table_name ." LEFT JOIN (SELECT correo, guia, pedido as pedido_p FROM paquetes) AS paquetes ON ". $parent_table_name .".pedido = paquetes.pedido_p WHERE ". $parent_table_name .".pedido not in (SELECT ". $table_name .".pedido from ". $table_name.") && ". $parent_table_name .".estado = 1". $filtro .";";
				...
				if(mysql_num_rows($consult)!=0){
					...
					while($row = mysql_fetch_assoc($consult)){
						...
						echo "<td><a href='#' onClick='addForm_registrar(this.parentNode.parentNode,\"". $row['pedido'] ."\",\"". $guia ."\"); return false;'>Imprimir</a></td>";
			...
			case 'RECIENTES': // Tabla Recientes
				...
				// Ensamblar Filtro
				if($filtro)
					$filtro = " WHERE ". $table_name .".pedido like '%". $filtro ."%' || ". $table_name .".encargado like '%". $filtro ."%' || ". $table_name .".comentario like '%". $filtro ."%' || paquetes.correo like '%". $filtro ."%' || paquetes.guia like '%". $filtro ."%'";
				
				//obtener el total de registros
				$sql="SELECT count(*) as total FROM ". $table_name ." LEFT JOIN (SELECT correo, guia, pedido as pedido_p FROM paquetes) AS paquetes ON ". $table_name .".pedido = paquetes.pedido_p". $filtro .";";
				$consult = mysql_query($sql, $conex) or die(mysql_error());
				$consult = mysql_fetch_assoc($consult);
				$total = $consult['total'];
				unset($consult);
				...
				//obtener datos entre los limites solicitados
				$sql="SELECT * FROM ". $table_name ." LEFT JOIN (SELECT correo, guia, pedido as pedido_p FROM paquetes) AS paquetes ON ". $table_name .".pedido = paquetes.pedido_p". $filtro ." ORDER BY ". $orderBy ." ". $orderBy_order ." LIMIT ". $consultas * $indice .", $consultas;";
				$consult = mysql_query($sql, $conex) or die(mysql_error());
				...
				if(mysql_num_rows($consult)!=0){
					...
					echo "<table id='Tabla-Resultado'><tr>";
						...
						if ($orderBy == "guia") echo "<th class='ordered , ". $orderBy_order ."'>";
						else echo "<th>";
						echo "<a href='?indice=". $indice ."&orderBy=guia'>Guía</a></th>";
					...
					while($row = mysql_fetch_assoc($consult)){
						...
						echo "<tr><td>".$row['fecha']."</td><td>".$row['pedido']."</td><td> ".$row['correo']." </td><td>".$row['encargado']."</td><td>".$guia."</td><td>".$row['asunto']."</td><td>".$row['comentario']."</td>";
						...
						else echo "<td><a href='#' onClick='addForm_registrar(this.parentNode.parentNode,\"". $row['pedido'] ."\",\"". $guia ."\"); return false;'>+</a></td>";
