	...
	Switch($ident){
		...
		case 'REGISTER': //operación de inserción
			...
			if(mysql_num_rows($paquete)==0)
				$sql[] = "INSERT INTO paquetes (pedido, correo, finicio, fultima, estado, vendedor) VALUES('$pedido', '$correo', '$timestamp', '$timestamp', '$estado_label','$usuario');";
			else {
				// sacar estado actual
...
<html>
	...
	<?php
		...
		switch($tabla) {
			...
			// Ensamblar Filtro
			if($filtro)
				$filtro = " WHERE a.pedido like '%". $filtro ."%' || a.encargado like '%". $filtro ."%' || a.comentario like '%". $filtro ."%' || b.correo like '%". $filtro ."%'";
			
			//obtener el total de registros
			$sql="SELECT count(*) as total FROM ". $table_name;
			if($filtro)
				$sql.=" AS a LEFT JOIN (SELECT correo, pedido AS pedido_p FROM paquetes) AS b ON a.pedido = b.pedido_p". $filtro .";";
			...
			$sql="SELECT * FROM ". $table_name ." AS a LEFT JOIN (SELECT correo, pedido AS pedido_p FROM paquetes) AS b ON (a.pedido = b.pedido_p)". $filtro ." ORDER BY ". $orderBy ." ". $orderBy_order ." LIMIT ". $consultas * $indice .", $consultas;";
