	...
	<?php
		...
		switch($tabla) {
			...
			case 'HEREDADOS': // Tabla Recientes
				...
				// Ensamblar Filtro
				if($filtro)
					$filtro = " && a.pedido like '%". $filtro ."%' || a.encargado like '%". $filtro ."%' || a.comentario like '%". $filtro ."%' || b.correo like '%". $filtro ."%'";
				
				//obtener el total de registros
				//obtener el total de registros
				$sql= "SELECT count(*) as total FROM ". $parent_table_name;
				if($filtro)
					$sql.= " AS a LEFT JOIN (SELECT correo, pedido AS pedido_p FROM paquetes) AS b ON a.pedido = b.pedido_p";
				$sql.= " WHERE pedido NOT IN (SELECT ". $table_name .".pedido from ". $table_name.") && estado = 1". $filtro .";";
