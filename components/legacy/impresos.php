	...
	<?php
		...
		switch($tabla) {
			...
			case 'HEREDADOS': // Tabla Recientes
				...
				// Ensamblar Filtro
				if($filtro)
					$filtro = " && ". $parent_table_name .".pedido like '%". $filtro ."%' || ". $parent_table_name .".encargado like '%". $filtro ."%' || ". $parent_table_name .".comentario like '%". $filtro ."%'";
				
				//obtener el total de registros
				$sql="SELECT count(*) as total FROM ". $parent_table_name ." WHERE ". $parent_table_name .".pedido not in (SELECT ". $table_name .".pedido from ". $table_name.") && ". $parent_table_name .".estado = 1". $filtro .";";
				...
				$sql="SELECT * FROM ". $parent_table_name ." WHERE ". $parent_table_name .".pedido not in (SELECT ". $table_name .".pedido from ". $table_name.") && ". $parent_table_name .".estado = 1". $filtro ." ORDER BY ". $parent_table_name .".". $orderBy ." ". $orderBy_order ." LIMIT ". $consultas * $indice .", $consultas;";
				$consult = mysql_query($sql, $conex) or die(mysql_error());
				...
				if(mysql_num_rows($consult)!=0){
					...
					while($row = mysql_fetch_assoc($consult)){
						...
						$sql = "SELECT correo,guia FROM paquetes WHERE paquetes.pedido = '".$row['pedido']."';";
			...
			case 'RECIENTES': // Tabla Recientes
				...
				// Ensamblar Filtro
				if($filtro)
					$filtro = " WHERE ". $table_name .".pedido like '%". $filtro ."%' || ". $table_name .".encargado like '%". $filtro ."%' || ". $table_name .".comentario like '%". $filtro ."%'";
				
				//obtener el total de registros
				$sql="SELECT count(*) as total FROM ". $table_name . $filtro .";";
				$consult = mysql_query($sql, $conex) or die(mysql_error());
				$consult = mysql_fetch_assoc($consult);
				$total = $consult['total'];
				unset($consult);
				...
				//obtener datos entre los limites solicitados
				$sql="SELECT * FROM ". $table_name . $filtro ." ORDER BY ". $table_name .".". $orderBy ." ". $orderBy_order ." LIMIT ". $consultas * $indice .", $consultas;";
				$consult = mysql_query($sql, $conex) or die(mysql_error());
