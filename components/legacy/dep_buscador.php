...
<html>
	...
	<body onload='setTimeout(limpiar(document.getElementById("buscador")),10);'>
		...
		if(!$pedido){
			...
			$sql="SELECT * FROM paquetes WHERE paquetes.pedido = '$pedido';";
			...
			if(mysql_num_rows($consult) != 0){
				...
				if($acceso || $consult['correo'] == $correo) {
					...
					echo "<table><tr><th width='100'>Progreso de la compra</th>";
					...
					// Encabezado tabla
					echo "<table id='historial'>";
					echo "<tr><th>Fecha</th><th>Pedido</th><th>Asunto</th><th>Comentario</th><th>Estado</th>";
					...
					$sql="SELECT * FROM cotejados WHERE cotejados.pedido = '$pedido' ORDER BY cotejados.fecha;";
					$consult = mysql_query($sql, $conex) or die(mysql_error());
					...
					if(mysql_num_rows($consult)!=0){
						...
						while($row = mysql_fetch_assoc($consult)){
							echo "<tr><td>".$row['fecha']."</td><td>".$row['pedido']."</td><td>".$row['asunto']."</td><td>".$row['comentario']."</td>";
					...
					*/
					...
					$sql="SELECT * FROM impresos WHERE impresos.pedido = '$pedido' ORDER BY impresos.fecha;";
					$consult = mysql_query($sql, $conex) or die(mysql_error());
					...
					$sql="SELECT * FROM separados WHERE separados.pedido = '$pedido' ORDER BY separados.fecha;";
					$consult = mysql_query($sql, $conex) or die(mysql_error());
					...
					$sql="SELECT * FROM revision WHERE revision.pedido = '$pedido' ORDER BY revision.fecha;";
					$consult = mysql_query($sql, $conex) or die(mysql_error());
					...
					$sql="SELECT * FROM cerrado WHERE cerrado.pedido = '$pedido' ORDER BY cerrado.fecha;";
					$consult = mysql_query($sql, $conex) or die(mysql_error());
					...
					$sql="SELECT * FROM recoleccion WHERE recoleccion.pedido = '$pedido' ORDER BY recoleccion.fecha;";
					$consult = mysql_query($sql, $conex) or die(mysql_error());
