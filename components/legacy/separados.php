...
-----------------------*/
	...
	$parent_table_name = 'impresos';
	$valid_states = array(
		"Artículos Separados"
		); // Estados definidos como aprovados para las publicaciones
	$estado_label_next = "Mercancía Apartada";
	$estado_label_act = "Pedido en Almacen";
...
<html>
		...
		<script type='text/javascript'>
			...
			function addForm_registrar(fila,num_pedido){
				...
				asunto.name = 'asunto';
					...
					option.value = 'Artículos Separados';
					option.appendChild(document.createTextNode('Artículos Separados'));
					asunto.appendChild(option);
					
					option = document.createElement('option');
					option.value = 'Artículos Faltantes';
					option.appendChild(document.createTextNode('Artículos Faltantes'));
					asunto.appendChild(option);
	...
	<?php
		...
		if($tabla == 'HEREDADOS') echo "<span>Impresiones Nuevas</span>";
		else echo "<a href='?tabla=HEREDADOS'>Impresiones Nuevas</a>";
		if($tabla == 'RECIENTES') echo "<span>Bitacora Separados</span>";
		else echo "<a href='?tabla=RECIENTES'>Bitacora Separados</a>";
			...
			case 'HEREDADOS': // Tabla Recientes
				...
				if(mysql_num_rows($consult)!=0){
					...
					while($row = mysql_fetch_assoc($consult)){
						...
						echo "<td><a href='#' onClick='addForm_registrar(this.parentNode.parentNode,\"". $row['pedido'] ."\"); return false;'>Separar</a></td>";
