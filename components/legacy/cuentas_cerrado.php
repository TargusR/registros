...
-----------------------*/
	...
	// filtro fechas
	$fmin = catchValue('fmin','GET');
	$fmax = catchValue('fmax','GET');
	
	// recordar fechas
	if($fmin)
		$_SESSION[$table_name]['fmin'] = $fmin;
	else if( isset($_SESSION[$table_name]['fmin']) )
		$fmin = $_SESSION[$table_name]['fmin'];
	
	if($fmax)
		$_SESSION[$table_name]['fmax'] = $fmax;
	else if( isset($_SESSION[$table_name]['fmax']) )
		$fmax = $_SESSION[$table_name]['fmax'];
...
<html>
	...
	<?php
		...
		if($tabla == 'CUENTAS') echo "<span>Cuentas Paquetes</span>";
		else echo "<a href='?tabla=CUENTAS'>Cuentas Paquetes</a>";
		...
		switch($tabla) {
			...
			case 'CUENTAS': // Tabla Recientes

				// Ensamblar Filtro
				if($filtro)
					$filtro = " && comentario like '%". $filtro ."%'";
				
				//echo "fecha mínima ".$fmin."<br>Fecha máxima ".$fmax;
				
				//Obtener variables de ordenación
				if(isset($_SESSION[$table_name]['orderBy'])) $orderBy = $_SESSION[$table_name]['orderBy'];
				else $orderBy = "comentario";
				
				if(isset($_SESSION[$table_name]['orderBy_order'])) $orderBy_order = $_SESSION[$table_name]['orderBy_order'];
				else $orderBy_order = "DESC";
				
				//$sql="select fecha, comentario, count(*) as cantidad from cerrado where fecha BETWEEN '".$fmin."'  AND '".$fmax."' group by comentario order by comentario ASC;";
				//$sql="select comentario, count(distinct pedido) as cantidad from cerrado where estado = 1 && fecha BETWEEN '".$fmin."'  AND '".$fmax."'". $filtro ." group by comentario order by ". $orderBy ." ". $orderBy_order .";";
				
				$sql = "select comentario, count(*) as cantidad from cerrado as a right join (select pedido, max(id) as ultimo from cerrado as a where estado = 1 && fecha BETWEEN '".$fmin."' AND '".$fmax."'". $filtro ." group by pedido) as b on (a.id = b.ultimo) where estado = 1 && fecha BETWEEN '".$fmin."'  AND '".$fmax."'". $filtro ." group by comentario order by ". $orderBy ." ". $orderBy_order .";";
				...
				if(mysql_num_rows($consult)!=0){
					...
					echo "<a href='?orderBy=comentario'>Encargado/detalles</a></th>";
					
					if ($orderBy == "cantidad") echo "<th class='ordered , ". $orderBy_order ."'>";
					else echo "<th>";
					echo "<a href='?orderBy=cantidad'>Cantidad</a></th>";
					
					echo "</tr>";
					
					while($row = mysql_fetch_assoc($consult)){
						
						echo "<tr><td>".$row['comentario']."</td><td>".$row['cantidad']."</td>";
						echo "</tr>";
						
					} echo '</table>';
				
				} else {
					echo "<div class='alert'><p>No hay cuentas entre esas fechas <a href='?'>Recargar</a></p></div>";
				}
