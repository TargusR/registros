...
-----------------------*/
	...
	$table_name = 'revision';
	$parent_table_name = 'separados';
	$valid_states = array(
		"Paquete Revisado"
		); // Estados definidos como aprovados para las publicaciones
	$estado_label_next = "Mercancía Revisada";
	$estado_label_act = "Mercancía Apartada";
...
<html>
		...
		<script type='text/javascript'>
			...
			function addForm_registrar(fila,num_pedido){
				...
				asunto.name = 'asunto';
					...
					option.value = 'Paquete Revisado';
					option.appendChild(document.createTextNode('Paquete Revisado'));
					asunto.appendChild(option);
					
					option = document.createElement('option');
					option.value = 'Paquete Incompleto';
					option.appendChild(document.createTextNode('Paquete Incompleto'));
					asunto.appendChild(option);
	...
	<body class='dobleCetrado' 
		<?php
		if(!$alert)
			echo "onLoad='setTimeout(limpiar(parent.document.getElementById(&quot;revision-form&quot;)),5);'";
		?>
	>
	...
	<?php
		...
		if($tabla == 'HEREDADOS') echo "<span>Nuevos Separados</span>";
		else echo "<a href='?tabla=HEREDADOS'>Nuevos Separados</a>";
		if($tabla == 'RECIENTES') echo "<span>Bitacora Revision</span>";
		else echo "<a href='?tabla=RECIENTES'>Bitacora Revision</a>";
			...
			case 'HEREDADOS': // Tabla Recientes
				...
				if(mysql_num_rows($consult)!=0){
					...
					while($row = mysql_fetch_assoc($consult)){
						...
						echo "<td><a href='#' onClick='addForm_registrar(this.parentNode.parentNode,\"". $row['pedido'] ."\"); return false;'>Revisar</a></td>";
