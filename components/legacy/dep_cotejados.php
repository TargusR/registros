...
-----------------------*/
	...
	$consultas = 16; // Número de registros recientes a mostrar por página
	$table_name = 'cotejados';
	$valid_states = array(
		"Pago Validado",
		"Saldo a Favor"
		); // Estados definidos como aprovados para las publicaciones
	$estado_label_next = "Pago revisado";
	$estado_label_act = "Revisando pago";
	...
	Switch($ident){
		...
		case 'REGISTER': //operación de inserción
			...
			// Alertar si falta correo
			if(!$correo) {
				// Contar registros previos
				$previos="SELECT count(*) as total FROM cotejados WHERE cotejados.pedido = '$pedido';";
				$consult = mysql_query($previos, $conex) or die(mysql_error());
				$consult = mysql_fetch_assoc($consult);
				$total = $consult['total'];
				unset($consult);
				
				if($total <= 0) {
					$msg = "<p>Error: Es necesario especificar un correo válido para registrar el pedido ". $pedido ."</p>";
					$alert = true;
					break;					
				}
			}
			...
			if(mysql_num_rows($paquete)==0)
				$sql[] = "INSERT INTO paquetes (pedido, correo, finicio, fultima, estado) VALUES('$pedido', '$correo', '$timestamp', '$timestamp', '$estado_label');";
			else {
				// sacar estado actual
...
<html>
		...
		<script type='text/javascript'>
			...
			function addForm_registrar(fila,num_pedido){
				...
				asunto.name = 'asunto';
					...
					option.value = 'Pago Validado';
					option.appendChild(document.createTextNode('Pago Validado'));
					asunto.appendChild(option);
					
					option = document.createElement('option');
					option.value = 'Pago Pendiente';
					option.appendChild(document.createTextNode('Pago Pendiente'));
					asunto.appendChild(option);
					
					option = document.createElement('option');
					option.value = 'Saldo a Favor';
					option.appendChild(document.createTextNode('Saldo a Favor'));
					asunto.appendChild(option);
	...
	<body class='dobleCetrado' 
		<?php
		if(!$alert)
			echo "onLoad='setTimeout(limpiar(parent.document.getElementById(&quot;cotejados-form&quot;)),5);'";
		?>
	>
	...
	<?php
		...
		if($tabla == 'RECIENTES') echo "<span>Bitacora Cotejados</span>";
		else echo "<a href='?tabla=RECIENTES'>Bitacora Cotejados</a>";
		...
		switch($tabla) {
			...
			// Ensamblar Filtro
			if($filtro)
				$filtro = " WHERE cotejados.pedido like '%". $filtro ."%' || cotejados.encargado like '%". $filtro ."%' || cotejados.comentario like '%". $filtro ."%' || paquetes.correo like '%". $filtro ."%'";
			
			//obtener el total de registros
			$sql="SELECT count(*) as total FROM cotejados LEFT JOIN (SELECT correo, pedido as pedido_p FROM paquetes) AS paquetes ON cotejados.pedido = paquetes.pedido_p". $filtro .";";
			...
			$sql="SELECT * FROM cotejados LEFT JOIN (SELECT correo, pedido as pedido_p FROM paquetes) AS paquetes ON cotejados.pedido = paquetes.pedido_p". $filtro ." ORDER BY ". $orderBy ." ". $orderBy_order ." LIMIT ". $consultas * $indice .", $consultas;";
			$consult = mysql_query($sql, $conex) or die(mysql_error());
			...
			if(mysql_num_rows($consult)!=0){
				...
				if($filtro)
					echo "<div class='alert'><p>No hay información para mostrar con ese Filtro :/ <a class='cmd' href='?filtro=unset'>Borrar Filtro</a></p></div>";
				else
